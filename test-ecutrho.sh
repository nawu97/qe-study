...
##SBATCH -p regular
#SBATCH -p debug
for ecut in 70 140 210 280 350 420 490 560 630 700 770 840
do
    echo "Running for ecutrho= $ecut ..."

    #self-consistent calculation
    cat > pw.NV.relaxed-test-ecutrho.in << EOF
&CONTROL
 calculation='scf',
...
EOF
	...
	srun --mpi=pmi2   $EXEC -i  pw.NV.relaxed-test-ecutrho.in | tee pw.NV.relaxed-test-ecutrho.out


    # collect the ecutwfc and total-energy from the pw.si.scf.out output-file
    grep -e 'charge density cutoff' -e ! pw.NV.relaxed-test-ecutrho.out | \
        awk '/kinetic-energy/ {ecut=$(NF-1)}
             /!/              {print ecut, $(NF-1)}' >> NV.test-ecutrho
